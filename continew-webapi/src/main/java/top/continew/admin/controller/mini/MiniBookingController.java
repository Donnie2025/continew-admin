...
public class MiniBookingController extends BaseController<BookingService, BookingResp, BookingDetailResp, BookingQuery, BookingReq> {
    ...
    public List<Long> createBooking(@Valid @RequestBody BatchBookingReq batchReq) {
        return baseService.createBatchBookingWithTransaction(batchReq);
    }

    @Operation(summary = "获取我的预约列表", description = "获取当前学生的所有预约记录")
    @GetMapping("/my")
    public List<MyBookingResp> getMyBookings() {
        return baseService.getMyBookings();
    }
    ...
    public List<BookingDO> getBookingsByMaterial(@Parameter(description = "教材ID") @PathVariable Long materialId,
        ...
        return baseService.listByStudentIdAndMaterialId(studentId, materialId);
    }
    ...
    public BookingDO getLastBooking() {
        return baseService.getLastBookingByCurrentStudent();
    }
    ...
    public void cancelBookingByTeacher(@Parameter(description = "预约ID") @PathVariable Long id) {
        baseService.cancelBookingByTeacher(id);
    }
